...
- **注册**：总控注册页由用户选节点（显示状态徽章 + 实测延迟，负载全 <50% 可选）→ Agent 从节点 adapter 上报带单调版本和短过期时间的注册/邀请码策略；未知、读取失败、版本回退、同版本改状态或过期一律不可注册 → Controller 建立持久注册 workflow，由 Agent 以稳定 registration ID 在节点幂等供应并消费节点自有邀请码 → 节点明确成功后才原子发布全局身份、节点账号和家副本。浏览器只持有 Strict/HttpOnly 状态 cookie，通过固定端点查询；响应丢失或结果不确定时不换节点、不提前建用户并安全重试。
...
- **身份管理**：一个全局用户最多各绑定一个密码、Discord、LinuxDo 身份且至少保留一种；OAuth 绑定 state 与当前用户/session/主控世代绑定。改密或新绑密码会将 verifier 和全部节点的版本化期望材料原子落库后再投递，离线/失败节点由持久材料后台重试。
