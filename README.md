...
- **自动续跑与滚动更新**：既有账号扫描按页持久化到 PostgreSQL；浏览器断开、短暂 502 或 Controller 换代后从最后确认页继续。可选的 `agent_auto_update` 默认只逐台更新无在线用户、无任务队列且已具备自更新能力的 Agent；启用 `allow_online_users` 后可在用户在线时仅重启 Agent（不会重启 SillyTavern）。Agent 固定从其 Controller 下载当前架构产物，复核 SHA-256 与内嵌版本后原子替换；服务不能稳定启动时由独立 transient unit 回滚上一二进制。首次从旧版引导到 `0.4.0` 仍需运行一次安装器升级。
...
Agent 0.4.6 起，relay 将密文拆成 16 MiB 的独立请求；Cloudflare/Nginx 只会看到远小于
100 MiB 的单个分片。上传或下载遇到 408/429/502/503/504 时只重试失败分片，并在服务端
校验每片 SHA-256、组合后的完整密文 SHA-256 和既有端到端 AEAD。旧版单请求接口保留供
兼容，但冲突合并会等待相关 Agent 升级到支持分片的版本后再开始中转。
...
- **备份**：Controller 每 30 秒判断一次；用户最后页面/请求活动和写租约都已超过 `backup.offline_grace_min` 后，才创建持久 workflow。随后单用户关写门并排空 → 同任务目录复制不可变快照 → manifest 先行的 tar.zst → 短期 capability 的 HTTPS 直传或端到端加密 relay → 目标限制窗口/文件数/大小/展开比并逐文件重算摘要 → 同文件系统换代发布。命令与 relay 下载使用短租约持续续期，正常大文件不会被截断，Agent/Controller 重启或 502 后几分钟内重领；已发布回执可幂等复用。失败或用户回归只清理未发布临时目录，旧成功副本保持不变。
