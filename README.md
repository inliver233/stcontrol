...
- **自动续跑与滚动更新**：既有账号扫描按页持久化到 PostgreSQL；浏览器断开、短暂 502 或 Controller 换代后从最后确认页继续。可选的 `agent_auto_update` 只逐台更新无在线用户、无任务队列且已具备自更新能力的 Agent，固定从其 Controller 下载当前架构产物，复核 SHA-256 与内嵌版本后原子替换；服务不能稳定启动时由独立 transient unit 回滚上一二进制。首次从旧版引导到 `0.4.0` 仍需运行一次安装器升级。
