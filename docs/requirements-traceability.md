...
| R09 | 默认活动计算副本 + 一个纯存储副本；只保留最新成功副本，不提供 PITR/误删回退 | 角色/配额调度、高低水位、冷却期 | 上报实际可用字节和分配配额 | 无额外核心改造 | 显示保护状态，不承诺历史恢复 | 副本来源、保留原因、保护级别、清理条件 | 存储满、迁移失败、旧副本保留、稳定后清理 | 默认和示例配置固定只保留 1 个成功副本，成功发布后物理换代并把旧 manifest 标为 deleted；Agent 现上报真实可用字节、受管分配量和配额，备份目标须通过连通/运营/兼容/容量门禁。目标仍按既有顺序选首个合格节点，保护级别、临时计算保护、长期未保护告警和恢复稳定后的安全清理未实现，`部分` |
...
| R20 | 所有控制台页面连接真实后端、权限、加载/失败/空状态/分页/筛选/审计/进度并响应式可用 | 分页和权限 API、结构化错误 | 上报真实进度 | 提供可观测状态 | 全页面/按钮逐项接线 | 审计/告警/任务可查询 | 组件、API、E2E、权限、响应式 | 节点管理页已接真实四维健康、窗口指标、真实磁盘/配额、在线用户/队列和维护切换，公开节点卡移除原始负载及内部原因码；更新节点会写结构化安全审计。生产构建通过，但全站错误/空状态、筛选、分页、响应式、前端组件与 E2E 测试仍未完成，`部分` |
