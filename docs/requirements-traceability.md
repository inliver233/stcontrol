...
- `go test -coverprofile=coverage/conflict_resolution.coverprofile ./...`：通过；总覆盖率 41.3%（Agent 48.8%、Controller 24.7%、Store 54.6%）。新增覆盖双份保留的源/目标路径隔离、长路径选择校验、缺少主来源时必须逐项选择及所有新增 Store 公共入口的非法输入；Controller/Store 新增状态机显著扩大语句数，覆盖率仍远低于最终 80% 门禁，不能据此宣称上线完成。
