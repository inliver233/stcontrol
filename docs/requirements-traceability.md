...
| R19 | 计算故障、存储故障、陈旧副本、真实分叉、恢复、节点退役/升级/损坏均进入明确状态机 | 自动编排、用户确认风险、分层告警 | 检测/隔离/按用户恢复/低频校验 | 单用户冻结/恢复接口 | 故障确认、恢复目标选择、冲突差异 UI | lifecycle、conflicts、alerts、integrity checks | 全故障矩阵和进程重启 | 计算 home 故障按热备/纯存储/无副本进入可接管、需恢复、不可恢复；热备可显式接管。archive 到计算恢复已有独立 operation/HMAC、持久 workflow/steps、账号供应隔离、重启认领、短期 capability、storage-to-compute 直传、目标原子发布回执和最终 home/权威副本/账号/票据/租约/保护状态单事务切换；失败不会改变旧 home/archive。真实 conflict 会锁存用户、短码和写租约，并在调和事务创建唯一开放案件、一次性固化检测时来源。普通业务 session 仍失败关闭，保留的或重新认证的身份令牌只能进入要求 conflict 双投影与开放案件的专用恢复路由；只读 API 返回节点/副本/immutable 证据规模而不泄露路径。存储故障会安全重建。SillyTavern 专用恢复账号 adapter 尚未挂载，逐文件冲突证据/差异、来源选择/有限合并和原子解冻、旧物理副本清理、节点退役/升级/损坏状态机及真实故障矩阵仍缺，`部分` |
| R20 | 所有控制台页面连接真实后端、权限、加载/失败/空状态/分页/筛选/审计/进度并响应式可用 | 分页和权限 API、结构化错误 | 上报真实进度 | 提供可观测状态 | 全页面/按钮逐项接线 | 审计/告警/任务可查询 | 组件、API、E2E、权限、响应式 | 节点管理页已接真实四维健康；用户选点页接入真实保护投影、immutable 恢复时间、热备接管和 archive 恢复目标 API。恢复面板有目标加载/空态、显式风险确认、session-stable operation ID、准备/传输/校验/发布/重试/失败进度，只有真实 succeeded 才刷新并登录；管理员有真实保护告警页。生产构建通过，但全站筛选/分页/响应式、前端组件与 E2E 测试仍未完成，`部分` |
