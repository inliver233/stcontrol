...
| R19 | 计算故障、存储故障、陈旧副本、真实分叉、恢复、节点退役/升级/损坏均进入明确状态机 | 自动编排、用户确认风险、分层告警 | 检测/隔离/按用户恢复/低频校验 | 单用户冻结/恢复接口 | 故障确认、恢复目标选择、冲突差异 UI | lifecycle、conflicts、alerts、integrity checks | 全故障矩阵和进程重启 | 计算 home 故障按热备/纯存储/无副本进入可接管、需恢复、不可恢复；用户可显式确认 immutable 热备接管，旧 home 变 stale。存储故障不冻结当前 home，安全离线后自动重建到纯存储节点，没有目标则保持未保护并告警；新副本发布前不降级旧副本。真实 conflict 会锁存并冻结用户、会话、短码及写租约。纯存储到计算恢复、冲突差异/来源选择/合并、旧物理副本清理、节点退役/升级/损坏状态机及真实故障矩阵仍缺，`部分` |
