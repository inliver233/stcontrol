...
| R09 | 默认活动计算副本 + 一个纯存储副本；只保留最新成功副本，不提供 PITR/误删回退 | 角色/配额调度、高低水位、冷却期 | 上报实际可用字节和分配配额 | 无额外核心改造 | 显示保护状态，不承诺历史恢复 | 副本来源、保留原因、保护级别、清理条件 | 存储满、迁移失败、旧副本保留、稳定后清理 | 默认和示例配置固定只保留 1 个成功副本，成功发布后物理换代并把旧 manifest 标为 deleted；Agent 上报真实磁盘/配额事实。Controller 现持久投影纯存储已保护、仅计算临时保护和未保护等状态，用户页展示安全说明，短暂未保护按可配置宽限期去重告警。目标选择仍较简单，存储自动修复和恢复稳定后的临时副本安全清理尚未实现，`部分` |
...
| R19 | 计算故障、存储故障、陈旧副本、真实分叉、恢复、节点退役/升级/损坏均进入明确状态机 | 自动编排、用户确认风险、分层告警 | 检测/隔离/按用户恢复/低频校验 | 单用户冻结/恢复接口 | 故障确认、恢复目标选择、冲突差异 UI | lifecycle、conflicts、alerts、integrity checks | 全故障矩阵和进程重启 | 新增持久用户保护投影与周期调和：计算 home 故障按合格热备/纯存储/无副本分别进入可接管、需恢复、不可恢复；存储故障不冻结当前 home，临时/未保护状态宽限后去重告警。用户确认最近恢复点及数据丢失风险后，可用稳定 operation ID 原子接管属于自己的 immutable 热备；旧 home/副本变 stale。真实 conflict 会锁存并冻结用户、会话、短码及写租约，禁止自动覆盖。存储自动修复/恢复、冲突差异与来源选择/合并、节点退役/升级/损坏状态机及真实故障矩阵仍缺，`部分` |
| R20 | 所有控制台页面连接真实后端、权限、加载/失败/空状态/分页/筛选/审计/进度并响应式可用 | 分页和权限 API、结构化错误 | 上报真实进度 | 提供可观测状态 | 全页面/按钮逐项接线 | 审计/告警/任务可查询 | 组件、API、E2E、权限、响应式 | 节点管理页已接真实四维健康；用户选点页接入真实保护投影和 immutable 恢复时间，A 仍有活动 writer 时提示返回 A，无活动 writer 才显示精确恢复点的显式接管确认，存在热备时不会自动跳走；管理员新增真实保护告警页。生产构建通过，但全站筛选/分页/响应式、前端组件与 E2E 测试仍未完成，`部分` |
...
- `go test -coverprofile=coverage/node_capacity.coverprofile ./...`：通过；总覆盖率 39.6%，已覆盖持久会话/OAuth、活动租约/票据、enrollment、命令租约/围栏、Agent 重启去重、密文载荷、兼容 scrypt、快照发布以及容量状态分支，但尚未达到最终 80% 覆盖率。
