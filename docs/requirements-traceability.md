...
| R22 | 安全和隐私：TLS、目录权限、日志脱敏；不记录聊天/API key/密码/token；恶意归档与权限绕过必测 | 安全头、限流、错误脱敏、审计摘要 | TLS 校验、日志脱敏、安全解包 | no-store/referrer/本地接口认证 | 不泄露内部拓扑/任务 ID | 最小化审计数据和保留策略 | token/nonce/路径/归档/日志扫描 | 已禁止打印主密钥、隐藏敏感模型字段、限制 Agent body、统一认证错误、持久化一次性 nonce；登录短码不进 URL。Controller cookie 使用 HttpOnly/Secure/SameSite，数据库仅存 session/CSRF digest，所有已登录写请求执行双提交 CSRF 校验。安全解包、TLS 服务端强制、日志扫描和权限绕过全套测试仍待补，`部分` |
