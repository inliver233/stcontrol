...
func TestChooseStorageRepairTargetUsesHealthyPureStorage(t *testing.T) {
	...
	target := chooseStorageRepairTarget(nodes, 1, nil, false)
	if target == nil || target.ID != 4 {
		t.Fatalf("target=%+v", target)
	}
	if target := chooseStorageRepairTarget(nodes, 4, nil, false); target == nil || target.ID != 3 {
		t.Fatalf("fallback target=%+v", target)
	}
	nodes[2].ControlMode = "independent"
	if target := chooseStorageRepairTarget(nodes, 1, nil, false); target == nil || target.ID != 3 {
		t.Fatalf("independent target was not excluded: %+v", target)
	}
	nodes[1].TransferURL = ""
	if target := chooseStorageRepairTarget(nodes, 1, nil, false); target != nil {
		t.Fatalf("addressless storage was selected without relay: %+v", target)
	}
	if target := chooseStorageRepairTarget(nodes, 1, nil, true); target == nil || target.ID != 3 {
		t.Fatalf("addressless storage was not selected with relay: %+v", target)
	}
}
