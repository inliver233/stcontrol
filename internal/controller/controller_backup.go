...
func (s *Server) controllerBackupPolicy() config.ControllerDisasterBackupPolicy {
	if s == nil || s.Cfg == nil || (s.Cfg.ControllerBackup == (config.ControllerDisasterBackupPolicy{})) {
		return config.DefaultController().ControllerBackup
	}
	return s.Cfg.ControllerBackup
}
func controllerBackupInterval(policy config.ControllerDisasterBackupPolicy) time.Duration {
	if policy.IntervalSec > 0 {
		return time.Duration(policy.IntervalSec) * time.Second
	}
	return 24 * time.Hour
}
...
func (s *Server) reconcileControllerBackupOnce(ctx context.Context) {
	if s == nil || s.Store == nil || len(s.secretKey) == 0 || !isUUID(s.workflowWorkerID) {
		return
	}
	policy := s.controllerBackupPolicy()
	if !policy.Enabled {
	...
	if _, err := s.Store.ReconcileControllerDisasterBackups(ctx, now, maxAttempts); err != nil {
		return
	}
	...
	if policy.PgDump {
		kind = store.ControllerBackupKindFull
	}
	...
	if err := s.executeControllerBackup(ctx, run); err != nil {
		return
	}
}
...
func (s *Server) executeControllerBackup(ctx context.Context, run *store.ControllerDisasterBackupRun) error {
	...
	fail := func(code string) {
		_ = s.Store.FailControllerDisasterBackup(
			ctx, claimed.OperationID, code, maxAttempts, time.Now().UTC(),
		)
	}
	target, err := s.Store.GetNodeByID(ctx, claimed.NodeID)
	if err != nil || target == nil || target.TransferURL == "" {
		fail("target_unavailable")
		return fmt.Errorf("controller backup target unavailable")
	}

	tempDir, err := os.MkdirTemp("", "ctrl-backup-")
	if err != nil {
		fail("temp_dir_failed")
		return err
	}
	defer os.RemoveAll(tempDir)

	if err := s.Store.MarkControllerDisasterBackupProgress(ctx, claimed.OperationID, store.ControllerBackupSnapshotting); err != nil {
		fail("snapshot_progress_failed")
		return err
	}
	...
	if claimed.BackupKind == store.ControllerBackupKindFull || claimed.BackupKind == store.ControllerBackupKindPGDump {
		if s.Cfg != nil && s.Cfg.DatabaseURL != "" {
			...
			if err := runPgDump(ctx, s.Cfg.DatabaseURL, pgDumpPath); err != nil {
				fail("pg_dump_failed")
	...
	configPath := ""
	if s.ConfigPath != "" {
		if info, statErr := os.Stat(s.ConfigPath); statErr == nil && !info.IsDir() {
	...
	if err := buildControllerBackupArchive(ctx, archivePath, pgDumpPath, configPath, manifestJSON, recoveryPath); err != nil {
		fail("archive_failed")
		return err
	}
	...
	if err := s.Store.MarkControllerDisasterBackupProgress(ctx, claimed.OperationID, store.ControllerBackupTransferring); err != nil {
		fail("transfer_progress_failed")
		return err
	}
	...
	}, deriveWorkflowOperationID(
		claimed.OperationID, fmt.Sprintf("receive-controller-backup:%d", claimed.Attempt),
	), 60*time.Second); err != nil {
		fail("target_prepare_failed")
		return fmt.Errorf("prepare controller backup target: %w", err)
	}

	if err := s.Store.MarkControllerDisasterBackupProgress(ctx, claimed.OperationID, store.ControllerBackupVerifying); err != nil {
		fail("verify_progress_failed")
		return err
	}
	...
	if err != nil || receipt == nil || !receipt.OK {
		fail("transfer_failed")
		return fmt.Errorf("stream controller backup: %w", err)
	}
	if receipt.ArchiveSHA256 != archiveDigest || receipt.TotalBytes != archiveSize {
		fail("verify_failed")
		return fmt.Errorf("controller backup verification mismatch")
	}

	if err := s.Store.MarkControllerDisasterBackupProgress(ctx, claimed.OperationID, store.ControllerBackupPublishing); err != nil {
		fail("publish_progress_failed")
		return err
	}
...
func runPgDump(ctx context.Context, dsn, outPath string) error {
	...
	if parsed.User == nil {
		return fmt.Errorf("database user missing from url")
	}
	password := ""
	if parsed.User != nil {
	...
	closeErr := out.Close()
	if err != nil {
		return fmt.Errorf("pg_dump exited: %v: %s", err, strings.TrimSpace(stderr.String()))
	}
	return closeErr
}
...
func buildControllerBackupArchive(ctx context.Context, archivePath, pgDumpPath, configPath string, manifestJSON []byte, recoveryPath string) error {
	...
	if err := tw.WriteHeader(&tar.Header{Name: controllerBackupManifestName, Mode: 0o400, Size: int64(len(manifestJSON)), Typeflag: tar.TypeReg}); err != nil {
		return fail(err)
	}
	...
	for name, path := range map[string]string{
		...
		if err := tw.WriteHeader(&tar.Header{Name: name, Mode: 0o600, Size: info.Size(), Typeflag: tar.TypeReg}); err != nil {
			_ = file.Close()
			return fail(err)
		}
...
func streamControllerBackup(ctx context.Context, targetTransferURL, operationID, capability, sha256Hex, archivePath string, size int64) (*protocol.ControllerBackupReceipt, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("controller backup target returned status %d", resp.StatusCode)
	}
	var receipt protocol.ControllerBackupReceipt
	if err := json.Unmarshal(data, &receipt); err != nil || !receipt.OK {
		return nil, fmt.Errorf("invalid controller backup target receipt")
	}
	return &receipt, nil
}
...
func controllerBackupTransferEndpoint(raw, operationID string) (string, error) {
	...
	if base.Scheme != "https" && !(base.Scheme == "http" && loopback) {
		return "", fmt.Errorf("controller backup transfer requires HTTPS")
	}
