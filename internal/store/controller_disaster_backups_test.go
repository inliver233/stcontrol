...
func TestScheduleControllerDisasterBackupSkipsWhenCovered(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT backup.id::text,backup.operation_id::text.*backup.state IN \('scheduled','retry_wait'\).*backup.next_attempt_at<=\$1`).
		WithArgs(now, 3).WillReturnError(sql.ErrNoRows)
	mock.ExpectCommit()
	...
	if run != nil {
		t.Fatalf("expected nil run when covered")
	}
	assertMockExpectations(t, mock)
}

func TestScheduleControllerDisasterBackupReturnsDueRetryAfterRestart(t *testing.T) {
	t.Parallel()
	st, mock, closeDB := newMockStore(t)
	defer closeDB()
	now := time.Date(2026, 8, 10, 12, 0, 0, 0, time.UTC)
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)SELECT generation FROM controller_epochs.*controller_rebuild_operations.*FOR SHARE`).
		WillReturnRows(sqlmock.NewRows([]string{"generation"}).AddRow(int64(7)))
	mock.ExpectQuery(`(?s)SELECT EXISTS.*controller_disaster_backups`).
		WithArgs(now.Add(-24 * time.Hour)).
		WillReturnRows(sqlmock.NewRows([]string{"exists"}).AddRow(true))
	mock.ExpectQuery(`(?s)SELECT backup.id::text,backup.operation_id::text.*backup.state IN \('scheduled','retry_wait'\).*backup.next_attempt_at<=\$1`).
		WithArgs(now, 3).
		WillReturnRows(sqlmock.NewRows([]string{
			"id", "operation_id", "node_id", "node_name", "state", "controller_generation",
			"backup_kind", "attempt", "next_attempt_at", "created_at", "updated_at",
		}).AddRow(
			"33333333-3333-4333-8333-333333333333",
			"11111111-1111-4111-8111-111111111111", int64(9), "storage-9",
			ControllerBackupRetryWait, int64(7), ControllerBackupKindFull, 1,
			now.Add(-time.Second), now.Add(-time.Hour), now.Add(-time.Minute),
		))
	mock.ExpectCommit()
	run, err := st.ScheduleControllerDisasterBackup(context.Background(), controllerBackupScheduleParams(now))
	if err != nil {
	...
	if run == nil || run.State != ControllerBackupRetryWait || run.Attempt != 1 {
		t.Fatalf("unexpected due retry: %+v", run)
	}
	assertMockExpectations(t, mock)
}
...
func TestScheduleControllerDisasterBackupCreatesRunOnEligibleTarget(t *testing.T) {
	...
	if run == nil || run.NodeID != 9 || run.State != ControllerBackupScheduled || run.ControllerGeneration != 7 {
...
func TestClaimControllerDisasterBackupClaimsScheduledRun(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)SELECT backup.id::text,backup.node_id,backup.state.*controller_rebuild_operations.*FOR UPDATE OF backup`).WithArgs("11111111-1111-4111-8111-111111111111").
	...
	if run == nil || run.State != ControllerBackupSnapshotting || run.Attempt != 1 {
		t.Fatalf("unexpected run: %+v", run)
	}
	assertMockExpectations(t, mock)
}
...
func TestClaimControllerDisasterBackupNoRowsNil(t *testing.T) {
	...
	params := ClaimControllerDisasterBackupParams{OperationID: "11111111-1111-4111-8111-111111111111", LeaseOwner: "22222222-2222-4222-8222-222222222222", LeaseTTL: 6 * time.Hour, MaxAttempts: 3, Now: now}
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)SELECT backup.id::text,backup.node_id,backup.state.*FOR UPDATE OF backup`).WithArgs("11111111-1111-4111-8111-111111111111").WillReturnError(sql.ErrNoRows)
	mock.ExpectCommit()
	run, err := st.ClaimControllerDisasterBackup(context.Background(), params)
	if err != nil {
		t.Fatalf("err=%v", err)
...
func TestClaimControllerDisasterBackupRejectsClosedRecoveryGate(t *testing.T) {
	t.Parallel()
	st, mock, closeDB := newMockStore(t)
	defer closeDB()
	now := time.Date(2026, 8, 10, 12, 0, 0, 0, time.UTC)
	params := ClaimControllerDisasterBackupParams{
		OperationID: "11111111-1111-4111-8111-111111111111",
		LeaseOwner:  "22222222-2222-4222-8222-222222222222",
		LeaseTTL:    6 * time.Hour, MaxAttempts: 3, Now: now,
	}
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)FROM controller_disaster_backups backup.*controller_rebuild_operations.*FOR UPDATE OF backup`).
		WithArgs(params.OperationID).WillReturnError(sql.ErrNoRows)
	mock.ExpectCommit()
	run, err := st.ClaimControllerDisasterBackup(context.Background(), params)
	if err != nil || run != nil {
		t.Fatalf("run=%+v err=%v, want closed-gate no-op", run, err)
	}
	assertMockExpectations(t, mock)
}
...
func TestCompleteControllerDisasterBackupRejectsBadInput(t *testing.T) {
	...
	if !errors.Is(err, ErrInvalidControllerDisasterBackup) {
...
func TestFailControllerDisasterBackupMovesToRetryWait(t *testing.T) {
	...
	mock.ExpectExec(`(?s)UPDATE controller_disaster_backups SET.*state=CASE WHEN.*retry_wait`).
		WithArgs("11111111-1111-4111-8111-111111111111", "target_unavailable", 1, 3, now.Add(60*time.Second), now).WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()
...
func TestFailControllerDisasterBackupMovesToFailedAtMaxAttempts(t *testing.T) {
	...
	mock.ExpectExec(`(?s)UPDATE controller_disaster_backups SET.*state=CASE WHEN.*failed`).
		WithArgs("11111111-1111-4111-8111-111111111111", "pg_dump_failed", 3, 3, now.Add(240*time.Second), now).WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()
...
func TestReconcileControllerDisasterBackupsSupersedesOlderPerNode(t *testing.T) {
	...
	mock.ExpectExec(`(?s)UPDATE controller_disaster_backups SET.*state=CASE WHEN attempt>=\$2 THEN 'failed' ELSE 'retry_wait' END.*lease_expired.*lease_until<=\$1`).
	...
	if touched != 2 {
		t.Fatalf("expected touched=2, got %d", touched)
	}
	assertMockExpectations(t, mock)
}
